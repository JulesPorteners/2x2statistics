...
void dfs(u64 n, u64* moves, u64* moves_size, bool one, bool inverse){
    if (distances[0][n] == 0){
        if (inverse){
            for (u64 i = (*moves_size) - 1; i < *moves_size; i--){ 
                switch (moves[i]){
        ...
        for (u64 i = 0; i < 9; i++){ 
            if (distances[0][graph[n][i]] < distances[0][n]){
                moves[*moves_size] = i;
                (*moves_size)++;
                dfs(graph[n][i], moves, moves_size, one, inverse);
                if (one){
                    break;
                }
                (*moves_size)--;
...
void solve(struct cube* c, bool one, bool inverse){
    u64 moves[GOD];
    u64 moves_size = 0;
    u64 n = hash_table_get(c)->index;
    dfs(n, moves, &moves_size, one, inverse);
}
